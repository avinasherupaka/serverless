...
describe('uploadArtifacts', () => {
  ...
  describe('#uploadFunctionsAndLayers()', () => {
    it('should upload the service .zip file to the S3 bucket', () => {
      return runServerless(Object.assign({ cwd: fixturesPaths.regular }, runServelessOptions)).then(
        serverless => {
          const uploadRequest = serverless
            .getProvider('aws')
            .request.args.find(
    ...
    it('should upload the service artifact to the S3 bucket', () => {
      return runServerless(
        Object.assign({ cwd: fixturesPaths.customArtifact }, runServelessOptions)
      ).then(serverless => {
        const uploadRequest = serverless
          .getProvider('aws')
          .request.args.find(
  ...
  describe('#uploadCustomResources()', () => {
    it('should upload the service artifact to the S3 bucket', () => {
      return runServerless(
        Object.assign({ cwd: fixturesPaths.customResource }, runServelessOptions)
      ).then(serverless => {
        const uploadRequest = serverless
          .getProvider('aws')
          .request.args.find(
